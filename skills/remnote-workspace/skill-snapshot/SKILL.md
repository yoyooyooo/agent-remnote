---
name: remnote
description: 'Use this skill for any RemNote-specific read/write or local plugin-loading task. Trigger it whenever the user mentions RemNote, Daily Note, 今日笔记, 当前 page/focus/selection, remId, 替换/清空/追加子级, queue/WS/plugin sync, sent=0, powerup/table/property changes, local Developer URL / plugin server (`plugin serve|ensure|status|logs|stop`), remote `apiBaseUrl` mode, recent activity recap, scenario package/schema/run surfaces (`scenario schema *`, `scenario builtin *`, `scenario run`), builtin workflows, or asks what `source_scope` / `target_ref` / time-range literals to pass, even if they only mention a scenario id, one CLI flag, “最近几天”, or only say “记到笔记里” / “查一下当前 page”. Prefer the shortest `agent-remnote` business command first, such as `plugin current --compact`, `rem replace`, `rem children replace|clear|append|prepend`, `daily write`, `rem outline`, `db recent`, `daily rem-id`, `scenario schema explain`, or `plugin ensure`. Prefer outline-first writing when the content is naturally outline-shaped; keep normal writing when it is not. Only escalate to `apply --payload` for true dependency chains, and only add wait/verify when the user explicitly asks or the next step depends on completion.'
---
...
1. `agent-remnote <family> <subcommand> --help`
2. 再读 `scenario schema explain` / builtin package / skill guidance
3. 最后才真正执行写入

不要靠记忆脑补未验证的 flag、var 名或 scope token。
...
- 它的目标是承接 `todo list -> query --preset` 的迁移。
- 目前只完成本地兼容，`apiBaseUrl` 模式仍返回稳定拒绝，直到 preset parity promotion 完成。
...
- 可先用下面的命令把 repo 内置 scenario 注入到用户目录：
