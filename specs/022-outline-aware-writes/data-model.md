...
| `write_style` | string | `normal` / `single_root_outline` / `expand_in_place` |

说明：

- `outline_suitable=false` 时允许正常写法
- `single_root_outline` 与 `expand_in_place` 是内部结论，不要求调用方显式选择
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `source_op_id` | TEXT PRIMARY KEY | registry 主键（来源操作） |
| `backup_rem_id` | TEXT | 对应 backup Rem id |
| `source_txn` | TEXT NOT NULL | 来源事务 |
| `source_op_type` | TEXT NOT NULL | 来源操作类型 |
...
| `source_anchor_id` | TEXT | 原锚点 Rem |
| `result_json` | TEXT NOT NULL | 结果快照 JSON |
...
| 参数 | 说明 |
| --- | --- |
| `--state pending\|orphan\|retained\|cleaned` | 默认 `orphan`（未指定 `--backup-rem-id` 时） |
| `--kind children_replace\|selection_replace` | 可选过滤 |
| `--older-than-hours <n>` | 最小年龄（小时，非负整数） |
| `--limit <n>` | 输出条数 |
| `--json` | 机器可读输出 |
...
| --- | --- |
| `--state pending\|orphan\|retained\|cleaned` | 默认 `orphan`（未指定 `--backup-rem-id` 时） |
| `--kind children_replace\|selection_replace` | 可选 |
| `--older-than-hours <n>` | 可选 |
| `--backup-rem-id <rem_id>` | 精确命中单个 backup artifact |
| `--max-delete-subtree-nodes <n>` | 按次覆盖前端安全删除阈值 |
| `--apply` | 真正执行删除 |
| `--json` | 机器可读输出 |
