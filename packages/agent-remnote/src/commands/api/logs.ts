...
export const apiLogsCommand = Command.make(
  'logs',
  { pidFile, file, lines: Options.integer('lines').pipe(Options.withDefault(200)), follow: Options.boolean('follow') },
    ...
    Effect.gen(function* () {
      ...
      if (!Number.isFinite(lines) || lines <= 0) {
        return yield* Effect.fail(
          new CliError({ code: 'INVALID_ARGS', message: '--lines must be a positive integer', exitCode: 2 }),
        );
      }
      if (cfg.format === 'ids') {
        return yield* Effect.fail(
      ...
      const logFilePath = resolveUserFilePath(file ?? pidInfo?.log_file ?? apiFiles.defaultLogFile());
      ...
      if (!exists) {
        return yield* Effect.fail(
          new CliError({
            ...
            exitCode: 1,
            hint: ['agent-remnote api start', 'agent-remnote api status'],
          }),
        );
      }
