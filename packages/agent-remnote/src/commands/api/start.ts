...
import { API_START_WAIT_DEFAULT_MS, startApiDaemon } from './_shared.js';
...
export const apiStartCommand = Command.make(
  'start',
  {
    host,
    port,
  ...
  ({ host, port, wait, pidFile, logFile, stateFile }) =>
    Effect.gen(function* () {
      const result = yield* startApiDaemon({ host, port, waitMs: wait, pidFile, logFile, stateFile });
      yield* writeSuccess({
        data: result,
