...
import { PLUGIN_SERVER_START_WAIT_DEFAULT_MS, startPluginServer } from './_shared.js';
...
export const pluginStartCommand = Command.make(
  'start',
  {
    host,
    port,
  ...
  ({ host, port, wait, pidFile, logFile, stateFile }) =>
    Effect.gen(function* () {
      const result = yield* startPluginServer({ host, port, waitMs: wait, pidFile, logFile, stateFile });
      yield* writeSuccess({
        data: result,
