...
export const stackStopCommand = Command.make('stop', {}, () =>
  Effect.gen(function* () {
    ...
    yield* apiFiles
      .deleteStateFile(apiPidInfo?.state_file ?? apiFiles.defaultStateFile())
    ...
    yield* supervisorState
      .deleteStateFile(
        resolveUserFilePath(daemonPidInfo?.state_file ?? path.join(path.dirname(daemonPidFile), 'ws.state.json')),
      )
      .pipe(Effect.catchAll(() => Effect.void));
