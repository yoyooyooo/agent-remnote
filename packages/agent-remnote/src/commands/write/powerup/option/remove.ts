...
import { ensureOptionMutationSupportedForOption } from '../../_optionRuntimeGuard.js';

import { writeCommonOptions } from '../../_shared.js';

export const writePowerupOptionRemoveCommand = Command.make(
  'remove',
  {
  ...
  ({ option, notify, ensureDaemon, wait, timeoutMs, pollMs, dryRun, priority, clientId, idempotencyKey, meta }) =>
    Effect.gen(function* () {
      ...
      yield* ensureOptionMutationSupportedForOption({ scopeLabel: 'powerup', optionId: option });

      const payloadSvc = yield* Payload;

      const op = yield* Effect.try({
        try: () => normalizeOp({ type: 'remove_option', payload: { optionId: option } }, payloadSvc.normalizeKeys),
        catch: (e) =>
          isCliError(e)
            ? e
            : new CliError({
