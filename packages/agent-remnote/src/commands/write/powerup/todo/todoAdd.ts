      ...
      if (status !== 'clear') {
        if (!schema.statusAttrId) {
          return yield* Effect.fail(
            new CliError({
              code: 'INVALID_ARGS',
              message: 'Todo status property not found for this tag. Use --values with explicit propertyId/optionId instead.',
              exitCode: 2,
        ...
        opsRaw.push({ type: 'set_cell_select', payload: { remId, propertyId: schema.statusAttrId, optionIds: optionId } });
