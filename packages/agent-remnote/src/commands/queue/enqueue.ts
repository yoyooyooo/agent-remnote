...
export const queueEnqueueCommand = Command.make(
  'enqueue',
  {
    payload: payloadSpec,
    notify: Options.boolean('notify'),
    ensureDaemon: Options.boolean('ensure-daemon'),
  ...
  ({ payload, notify, ensureDaemon, priority, clientId, idempotencyKey, meta }) =>
    Effect.gen(function* () {
