...
describe('cli contract: runtime owner api metadata', () => {
  it('exposes source-tree owner metadata through api status', async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'agent-remnote-runtime-owner-api-'));
    ...
    try {
      const startRes = await runCli(
        ['--json', 'api', 'start', '--port', String(port), '--pid-file', pidFile, '--log-file', logFile, '--state-file', stateFile],
        { env: { HOME: tmpHome, REMNOTE_STORE_DB: storeDb, REMNOTE_TMUX_REFRESH: '0' }, timeoutMs: 30_000 },
      );
      ...
      try {
        await runCli(['--json', 'api', 'stop', '--pid-file', pidFile, '--state-file', stateFile], {
          env: { HOME: tmpHome, REMNOTE_STORE_DB: storeDb, REMNOTE_TMUX_REFRESH: '0' },
          timeoutMs: 30_000,
        });
      } catch {}
