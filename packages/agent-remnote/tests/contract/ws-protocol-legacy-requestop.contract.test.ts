...
describe('ws protocol contract: legacy RequestOp is rejected', () => {
  it('returns an Error with a stable code', async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'agent-remnote-ws-protocol-'));
    try {
      ...
      const program = Effect.gen(function* () {
        ...
        yield* Effect.promise(async () => {
          ...
          try {
            ...
            ws.send(JSON.stringify({ type: 'RequestOp', leaseMs: 1000 }));
            const err = await q.next((m) => m?.type === 'Error' && m?.code === 'WS_PROTOCOL_LEGACY_REQUEST_OP');
            expect(String(err.message || '')).toBe('Legacy RequestOp is not supported');
