...
import { RemDb, RemDbLive } from '../../src/services/RemDb.js';
...
describe('RemDb remote guard (unit)', () => {
    ...
    const result = await Effect.runPromise(
      Effect.either(
        Effect.gen(function* () {
          ...
          return yield* remDb.withDb('/tmp/remnote.db', () => 'ok');
        }).pipe(Effect.provide([cfgLayer, RemDbLive])),
      ),
    );
